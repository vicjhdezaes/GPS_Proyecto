...
def test_sumar(client):
    response = client.get('/sumar', query_string=dict(a=3, b=4))
    data = json.loads(response.data)
    assert response.status_code == 200
    assert data['resultado'] == 7
...
def test_restar(client):
    ...
    response = client.get('/restar', query_string=dict(a=3, b=4))
    data = json.loads(response.data)
    assert response.status_code == 200
...
def test_multiplicar(client):
    ...
    response = client.get('/multiplicar', query_string=dict(a=3, b=4))
    data = json.loads(response.data)
    assert response.status_code == 200
...
def test_dividir(client):
    ...
    response = client.get('/dividir', query_string=dict(a=3, b=4))
    data = json.loads(response.data)
    assert response.status_code == 200
